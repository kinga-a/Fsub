...
function renderDashboard() {
  ...
  subs.forEach(s => {
    if (s.enabled !== false && s.price > 0) {
      ...
      const monthlyCost = daily * 30 * (fx[s.currency] || 1);
  ...
  document.getElementById('topExpense').innerHTML = sorted.map((s, i) => {
    const sym = { CNY: '¥', USD: '$', EUR: '€', GBP: '£', JPY: '¥' };
    const totalDays = (daysInPeriod[s.cycleUnit] || 30) * (s.cycleValue || 1);
    const monthlyAmt = (s.price / totalDays) * 30 * (fx[s.currency] || 1);
    return '<div style="display:flex;align-items:center;gap:12px;padding:10px;background:#f8fafc;border-radius:8px;">' +
      ...
      '<div style="text-align:right;">' +
        ...
        '<div style="font-size:12px;color:var(--text-secondary);">≈ ¥' + monthlyAmt.toFixed(0) + '/月</div>' +
      '</div>' +
    '</div>';
  }).join('') || '<div style="text-align:center;color:var(--text-secondary);padding:20px;">暂无支出数据</div>';
}
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (daySubs.length > 0) {
      ...
      if (hasExpired) cellClass += ' calendar-cell-expired';
      else if (hasSoon) cellClass += ' calendar-cell-soon';
      else cellClass += ' calendar-cell-normal';
    }
    ...
    if (daySubs.length > 0) {
      subListHtml = '<div class="sub-list">' +
        daySubs.slice(0, 3).map(s => {
          return '<div class="sub-list-item" title="' + esc(s.name) + '">' + getTypeIcon(s.type) + ' ' + esc(s.name) + '</div>';
        }).join('') +
        (daySubs.length > 3 ? '<div class="sub-list-more">+' + (daySubs.length - 3) + '个</div>' : '') +
        '</div>';
    }
    ...
    html += '<div class="' + cellClass + '" ' + cellData + ' ontouchstart="handleCellTouch(event)" onmouseenter="showCalendarTooltip(event)" onmouseleave="hideCalendarTooltip(event)">' +
      '<div class="calendar-cell-day' + (isToday ? ' today-text' : '') + '">' + day + (isToday ? '<span class="today-label">今天</span>' : '') + '</div>' +
      subListHtml +
      dotRowHtml +
    '</div>';
  }
